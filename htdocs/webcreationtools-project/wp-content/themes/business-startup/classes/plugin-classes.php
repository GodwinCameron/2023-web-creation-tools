<?php
if ( !class_exists('Business_Startup_Getting_started') ):

    class Business_Startup_Getting_started
    {   

        function __construct()
        {	

	        add_action( 'wp_ajax_business_startup_install_plugins', array( $this, 'business_startup_install_plugins' ) );
        ...
        public static function business_startup_plugin_status($plugin_class, $plugin_folder, $plugin_file){

            if( class_exists( $plugin_class ) ){

                return array('status' => 'active','string' => esc_html__('Deactivate','business-startup') );
                ...
                if( file_exists( $path ) ) {
                    ...
                    return array('status' => 'deactivate','string' => esc_html__('Activate','business-startup') );
                }else{

                    return array('status' => 'not-install','string' => esc_html__('Install & Active','business-startup') );
        ...
        public static function business_startup_recommended_plugins(){
            ...
            return $plugin_lists = array(
                ...
                'wp-google-maps' => array(
                    'PluginFile' => 'wpGoogleMaps.php',
                    'class' => 'Map',
                    'setting_page' => esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=wp-google-maps-menu' ),
                ),
                'elementor' => array(
                    'PluginFile' => 'elementor.php',
                    'class' => 'Element_Base',
                    'setting_page' => esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=elementor' ),
                ),
                'contact-form-7' => array(
                    'PluginFile' => 'wp-contact-form-7.php',
                    'class' => 'WPCF7_ContactForm',
                    'setting_page' => esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=wpcf7' ),
                ),
            );
        ...
        public function business_startup_install_plugins(){
            
            $nonce = isset( $_POST["_wpnonce"] ) ? sanitize_text_field( wp_unslash( $_POST["_wpnonce"] ) ) : '';

            if ( ! current_user_can('install_plugins') ) {
                wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'business-startup' ) );
            }

            // Check our nonce, if they don't match then bounce!
            if (! wp_verify_nonce( $nonce, 'business_startup_ajax_nonce' )) {

                wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'business-startup') );
            ...
            if( isset( $_POST['single'] ) ){
                ...
                $plugin_lists = $this->business_startup_recommended_plugins();
            ...
            foreach( $plugin_lists as $key => $plugin ){
                ...
                if( isset( $_POST['single'] ) ){
                    ...
                    $pluginstatus   = $this->business_startup_plugin_status( $plugin['class'],$key,$plugin['PluginFile'] );
                ...
                if( isset( $_POST['single'] ) && $PluginStatus == 'active' ) {
                    ...
                    esc_html_e('Deactivated' , 'business-startup');
                    die();
                    ...
                    if( isset( $_POST['single'] ) ) {
                        ...
                        esc_html_e('Activated' , 'business-startup');
                        die();
                    ...
                    if( file_exists($plugin_dir) ) {
                        ...
                        if( isset( $_POST['single'] ) ) {
                            esc_html_e('Installed & Activated' , 'business-startup');
                            die();
                        }
                    ...
                    if( isset( $_POST['single'] ) ) {
                        ...
                        esc_html_e('Failed' , 'business-startup');
                        die();
            ...
            if( !isset( $_POST['single'] ) ) {
                echo esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=business-startup-about' );
            }
    ...
    new Business_Startup_Getting_started();
