...
function business_startup_scripts()
{
    ...
    wp_style_add_data('business-startup-style', 'rtl', 'replace');
...
{   
    wp_enqueue_style('business-startup-admin', get_template_directory_uri() . '/assets/twp/css/admin.css', array(), '1.0.0');
    if ('widgets.php' === $hook) {
        wp_enqueue_media();
    ...
    if( $current_screen->id != "widgets" ) {
        ...
        wp_enqueue_script('business-startup-admin', get_template_directory_uri() . '/assets/twp/js/admin.js', array('jquery'), '', 1);

        $ajax_nonce = wp_create_nonce('business_startup_ajax_nonce');
                
        wp_localize_script( 
            'business-startup-admin',
            'business_startup_admin',
            array(
                'ajax_url'   => esc_url( admin_url( 'admin-ajax.php' ) ),
                'ajax_nonce' => $ajax_nonce,
                'upload_image'   =>  esc_html__('Choose Image','business-startup'),
                'use_image'   =>  esc_html__('Select','business-startup'),
                'active' => esc_html__('Active','business-startup'),
                'deactivate' => esc_html__('Deactivate','business-startup'),
             )
        );
...
add_filter('themeinwp_enable_demo_import_compatiblity','business_startup_demo_import_filter_apply');

if( !function_exists('business_startup_demo_import_filter_apply') ):

    function business_startup_demo_import_filter_apply(){
